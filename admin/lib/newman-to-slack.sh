...
main () {
    ...
    # skipping since we run newman out of docker
    #validate_install
    ...
    # call newman
    local output=$(docker run --rm=true -v $(pwd):/etc/newman -t postman/newman_ubuntu1404:3.2.0 run $collection -e $environment $url $global $additional $newman_args)
    ...
    # post to slack
    curl -X POST --data-urlencode 'payload={"text": "'"Test results for <$JOB_URL|$JOB_NAME>"':```'"$output"'```"}' $webhook
}
